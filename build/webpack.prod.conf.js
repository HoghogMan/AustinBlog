...
const webpackConfig = merge(baseWebpackConfig, {
  ...
  plugins: [
    ...
    // 将CSS抽离出来 存放在单独的CSS文件夹下
    new ExtractTextPlugin({
      filename: utils.assetsPath('css/[name].[contenthash].css'),
      // Setting the following option to `false` will not extract CSS from codesplit
      // chunks. Their CSS will instead be inserted dynamically with style-loader when
      // the codesplit chunk has been loaded by webpack. It's currently set to `true`
      // because we are seeing that sourcemaps are included in the codesplit bundle
      // as well when it's `false`, increasing file size:
    ...
    // This instance extracts shared chunks from code splitted chunks and bundles
    // them in a separate chunk, similar to the vendor chunk see:
    ...
    //       foo: 'bar'
    //     },
    //     renderAfterTime: 10000, // 触发渲染的时间，用于获取数据后再保存渲染结果
    //     // headless: false,//如果项目部署在linux/centOS之类没有桌面的系统,无需headless: false去掉 在 main.js
    //     // 中 document.dispatchEvent(new Event('render-event'))，两者的事件名称要对应上。
    ...
    //         }
    //       },
    //       // proxy的属性用于express: app.use(key, proxy[key]) '/:foo': { target }, 可以不匹配 '/'
    //     }
    //   }
    // })
  ]
